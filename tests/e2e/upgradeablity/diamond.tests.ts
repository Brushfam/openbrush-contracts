...
describe('DIAMOND_PSP22', () => {
  ...
  it('Adding facets works', async () => {
    ...
    expect((await diamondContract.query.owner()).value.unwrapRecursively()).to.be.eq(defaultSigner.address)
    ...
    expect((await proxyPSP22.query.balanceOf(defaultSigner.address)).value.unwrapRecursively().toNumber()).to.eq(1000)
    ...
    // add metadata facet
    await diamondContract.withSigner(defaultSigner).tx.diamondCut(metadataCut, { hash: metadataHash, selector: metadataInit, input: [] })

    // patch methods
    const proxyPSP22Metadata = setupProxy(metadataFacet, diamondContract.address)

    expect((await proxyPSP22Metadata.query.tokenName()).value.unwrapRecursively()).to.eq('PSP22 Diamond')
    expect((await proxyPSP22Metadata.query.tokenSymbol()).value.unwrapRecursively()).to.eq('PSP22D')
    expect((await proxyPSP22Metadata.query.tokenDecimals()).value.unwrapRecursively()).to.eq(18)

    // Test Loupe

    console.log()

    expect((await diamondContract.query.facets()).value.unwrapRecursively()).to.be.deep.equal([
    ...
    expect((await diamondContract.query.facetFunctionSelectors(metadataHash)).value.unwrapRecursively()).to.be.deep.equal(
      getSelectorsFromMessagesString(metadataMessages)
    )
    expect((await diamondContract.query.facetFunctionSelectors(psp22Hash)).value.unwrapRecursively()).to.be.deep.equal(getSelectorsFromMessagesString(psp22Messages))

    expect((await diamondContract.query.facetCodeHashes()).value.unwrapRecursively()).to.be.deep.equal([psp22Hash, metadataHash])

    expect((await diamondContract.query.facetCodeHash(psp22Init)).value.unwrapRecursively()).to.eq(psp22Hash)
    expect((await diamondContract.query.facetCodeHash(metadataInit)).value.unwrapRecursively()).to.eq(metadataHash)
  ...
  it('Only owner can call diamond cut', async () => {
    ...
    // add psp22 facet
    await expect(diamondContract.withSigner(wrongSigner).tx.diamondCut(facetCut, { hash: psp22Hash, selector: initSelector, input: [] })).to
  ...
  it('Replacing facets works', async () => {
    ...
    expect((await diamondContract.query.facetFunctionSelectors(psp22NewHash)).value.unwrapRecursively()).to.be.deep.equal(getSelectorsFromMessagesString(messagesNew))
    expect((await diamondContract.query.facetFunctionSelectors(psp22Hash)).value.unwrapRecursively()).to.be.deep.equal([])

    expect((await diamondContract.query.facetCodeHashes()).value.unwrapRecursively()).to.be.deep.equal([psp22NewHash])
  ...
  it('Removing facets works', async () => {
    ...
    expect((await diamondContract.query.facetCodeHashes()).value.unwrapRecursively()).to.be.deep.equal([])
  ...
  it('Removing last facet will leave us first facet', async () => {
    ...
    const metadataSelectors = getSelectorsFromMessages(
      metadataMessages.filter((message) => {
        return (
          message.identifier != 'Ownable::owner' &&
          message.identifier != 'Ownable::renounce_ownership' &&
          message.identifier != 'Ownable::transfer_ownership'
        )
      })
    )
    ...
    // add metadata facet
    await expect(diamondContract.withSigner(defaultSigner).tx.diamondCut(metadataCut, { hash: metadataHash, selector: metadataInit, input: [] })).to
    ...
    expect((await diamondContract.query.facetCodeHashes()).value.unwrapRecursively()).to.be.deep.equal([psp22Hash])

    expect((await diamondContract.query.facetFunctionSelectors(psp22Hash)).value.unwrapRecursively()).to.be.deep.equal(getSelectorsFromMessagesString(messages))
  ...
  it('Removing first facet will leave us last facet', async () => {
    ...
    expect((await diamondContract.query.facetCodeHashes()).value.unwrapRecursively()).to.be.deep.equal([metadataHash])
    ...
    expect((await diamondContract.query.facetCodeHash(metadataInit)).value.unwrapRecursively()).to.be.deep.equal(metadataHash)
  ...
  it('Can not call function after removing it', async () => {
    ...
    // add psp22 facet
    await expect(diamondContract.withSigner(defaultSigner).tx.diamondCut(psp22Cut, { hash: psp22Hash, selector: psp22Init, input: [] })).to.eventually
      .be.fulfilled
    ...
    expect((await diamondContract.query.facetFunctionSelectors(psp22Hash)).value.unwrapRecursively()).to.be.deep.equal(
      getSelectorsFromMessagesString(psp22Messages).filter((selector) => {
