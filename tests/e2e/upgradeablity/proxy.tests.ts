...
describe('MY_UPGRADEABLE_PSP22', () => {
  ...
  it('MY_UPGRADEABLE_PSP22 - update psp22 to psp22_metadata', async () => {
    ...
    const tokenName = (await proxy_metadata.query.tokenName()).value.unwrapRecursively()?.toString()
    const tokenSymbol = (await proxy_metadata.query.tokenSymbol()).value.unwrapRecursively()?.toString()
    const totalSupply = (await proxy_metadata.query.totalSupply()).value.unwrapRecursively().toNumber()
    const tokenDecimals = (await proxy_metadata.query.tokenDecimals()).value.unwrapRecursively()
    const balance = (await proxy_metadata.query.balanceOf(receiver.address)).value.unwrapRecursively().toNumber()

    expect(totalSupply).to.be.eq(1000)
    expect(tokenName).to.be.eq('COLONY')
    expect(tokenSymbol).to.be.eq('COL')
